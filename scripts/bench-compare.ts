...
//   * @polyglot-sql/sdk is a Rust/WASM SQL parser and transpiler. We
//     run it with its SQLite dialect, and its ESM top-level WASM init
//     happens before the hot loop.
...
// happy.
function readNodeModulePkg(specifier: string): PkgInfo {
  return JSON.parse(
    readFileSync(new URL(`../node_modules/${specifier}/package.json`, import.meta.url), "utf8"),
  ) as PkgInfo
}

const sqlParserTsPkg = readNodeModulePkg("@guanmingchiu/sqlparser-ts")
const polyglotPkg = readNodeModulePkg("@polyglot-sql/sdk")
...
function polyglotParse(sql: string): unknown {
