...
describe('numbers', () => {
  test('integers', () => {
    expect(lexNames('0 1 42 1234567890'))
      .toEqual(['INTEGER', 'INTEGER', 'INTEGER', 'INTEGER']);
  });
...
describe('comments and whitespace', () => {
  ...
  test('trivia kept when skipTrivia:false', () => {
    expect(lexNames('SELECT -- c\n1', { skipTrivia: false }))
      .toEqual(['SELECT', 'SPACE', 'COMMENT', 'SPACE', 'INTEGER']);
  });

  test('/* block comment */', () => {
    expect(lexNames('/* hi */ SELECT', { skipTrivia: false }))
      .toEqual(['COMMENT', 'SPACE', 'SELECT']);
  });

  test('unterminated /* block comment is still COMMENT (consumed to EOI)', () => {
    expect(lexNames('/* never closes', { skipTrivia: false }))
      .toEqual(['COMMENT']);
  });
});
...
describe('operators', () => {
  test('arithmetic', () => {
    expect(lexNames('1+2-3*4/5%6')).toEqual([
      ...
      'STAR',    'INTEGER', 'SLASH', 'INTEGER', 'REM', 'INTEGER',
    ]);
  });
  ...
  test('grouping and punctuation', () => {
    expect(lexNames('(a, b);'))
      .toEqual(['LP', 'ID', 'COMMA', 'ID', 'RP', 'SEMI']);
  });
});
...
describe('variables', () => {
  ...
  test('named $foo, @foo, :foo, #foo', () => {
    expect(lex('$x @y :z #w').map((t) => t.text))
      .toEqual(['$x', '@y', ':z', '#w']);
  });
});
...
describe('blob literals', () => {
  test("x'48656C6C6F' is a BLOB", () => {
    expect(lex("x'48656C6C6F'"))
      .toEqual([{ name: 'BLOB', text: "x'48656C6C6F'" }]);
  });
...
describe('keyword aliases', () => {
  test('GLOB / LIKE / REGEXP all collapse to LIKE_KW', () => {
    expect(lexNames('LIKE GLOB REGEXP'))
      .toEqual(['LIKE_KW', 'LIKE_KW', 'LIKE_KW']);
  });
  test('CROSS/FULL/INNER/LEFT/NATURAL/OUTER/RIGHT collapse to JOIN_KW', () => {
    expect(lexNames('CROSS FULL INNER LEFT NATURAL OUTER RIGHT'))
      .toEqual(['JOIN_KW', 'JOIN_KW', 'JOIN_KW', 'JOIN_KW', 'JOIN_KW', 'JOIN_KW', 'JOIN_KW']);
  });
  test('CURRENT_DATE/_TIME/_TIMESTAMP collapse to CTIME_KW', () => {
    expect(lexNames('CURRENT_DATE CURRENT_TIME CURRENT_TIMESTAMP'))
      .toEqual(['CTIME_KW', 'CTIME_KW', 'CTIME_KW']);
  });
});
