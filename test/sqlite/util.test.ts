...
describe("sqlite3DequoteNumber", () => {
  ...
  test("1_000 → 1000 (INTEGER)", () => {
    expect(sqlite3DequoteNumber("1_000")).toEqual({ op: "INTEGER", text: "1000" })
  })
  test("1.1_1 → 1.11 (FLOAT)", () => {
    expect(sqlite3DequoteNumber("1.1_1")).toEqual({ op: "FLOAT", text: "1.11" })
  })
  test("12_3_456.7_8_9 → 123456.789 (FLOAT)", () => {
    expect(sqlite3DequoteNumber("12_3_456.7_8_9")).toEqual({ op: "FLOAT", text: "123456.789" })
  })
  test("1e1_000 → 1e1000 (FLOAT → Infinity via Number)", () => {
    expect(sqlite3DequoteNumber("1e1_000")).toEqual({ op: "FLOAT", text: "1e1000" })
    expect(Number("1e1000")).toBe(Infinity) // downstream parsing
  })
  test("9_223_372_036_854_775_807 (INT64_MAX)", () => {
    expect(sqlite3DequoteNumber("9_223_372_036_854_775_807")).toEqual({
      op: "INTEGER",
  ...
  test("custom digitSeparator character", () => {
    expect(sqlite3DequoteNumber("1\u00a0000", { digitSeparator: "\u00a0" })).toEqual({
      op: "INTEGER",
      text: "1000",
    })
  })
...
describe("sqlite3HexToBlob", () => {
  ...
  test("mixed case hex digits", () => {
    expect(Array.from(sqlite3HexToBlob("x'aBcDEf'"))).toEqual([0xab, 0xcd, 0xef])
  })
...
describe("sqlite3Atoi64", () => {
  ...
  test("INT64_MAX", () => {
    expect(sqlite3Atoi64("9223372036854775807")).toEqual({
  ...
  test("INT64_MIN (exact, via minus prefix)", () => {
    ...
    expect(sqlite3Atoi64("-9223372036854775808")).toEqual({
...
describe("sqlite3DecOrHexToI64", () => {
  test("0xFF", () => {
    expect(sqlite3DecOrHexToI64("0xFF")).toEqual({ value: 255n, rc: ATOI_OK })
  })
  test("0xFFFFFFFFFFFFFFFF wraps to -1 (memcpy semantics)", () => {
    expect(sqlite3DecOrHexToI64("0xFFFFFFFFFFFFFFFF")).toEqual({ value: -1n, rc: ATOI_OK })
  })
  test("0x123FFFFFFFF", () => {
    expect(sqlite3DecOrHexToI64("0x123FFFFFFFF")).toEqual({ value: 0x123ffffffffn, rc: ATOI_OK })
  })
  ...
  test("decimal path delegates to Atoi64", () => {
    expect(sqlite3DecOrHexToI64("123")).toEqual({ value: 123n, rc: ATOI_OK })
  })
})
