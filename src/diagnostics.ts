...
export function formatDiagnostic(ctx: ParseErrorContext, diagnostic: Diagnostic): string {
  const { filename } = ctx
...
class ParserDiagnostic implements ParseDiagnostic {
  readonly #diagnostic: Diagnostic
  #context: ParseErrorContext | undefined
  #formatted: string | undefined
  #filename: string | undefined

  constructor(ctx: ParseErrorContext, diagnostic: Diagnostic) {
    if (!ctx) {
      throw new TypeError(`Expected a ParseErrorContext, got ${typeof ctx} (${ctx})`)
    }
  ...
  format(): string {
    ...
    if (!this.#context) {
      throw new Error("ParserDiagnostic: context is required to format a diagnostic")
    }
    this.#formatted = formatDiagnostic(this.#context, this.#diagnostic)
    // De-reference context now that we've formatted it to release memory
...
export function createParseDiagnosticArray(
  context: ParseErrorContext,
  diagnostics: readonly Diagnostic[],
): readonly ParseDiagnostic[] {
  return diagnostics.map((diagnostic) => new ParserDiagnostic(context, diagnostic))
}
