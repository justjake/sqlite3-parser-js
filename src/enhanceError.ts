...
const ITEM_STARTER_TOKEN_NAMES = new Set([
  'ID', 'STRING', 'INTEGER', 'FLOAT', 'QNUMBER', 'BLOB', 'VARIABLE',
  'LP', 'STAR', 'NULL', 'CASE', 'CAST', 'EXISTS', 'NOT',
  'PLUS', 'MINUS', 'BITNOT',
  'SELECT', 'VALUES', 'CURRENT', 'CTIME_KW',
]);

const ITEM_ENDER_TOKEN_NAMES = new Set([
  'ID', 'STRING', 'INTEGER', 'FLOAT', 'QNUMBER', 'BLOB', 'VARIABLE',
  'RP', 'NULL', 'TRUEFALSE', 'CURRENT', 'CTIME_KW', 'END',
]);

const CLAUSE_BOUNDARY_TOKEN_NAMES = new Set([
  'FROM', 'WHERE', 'GROUP', 'HAVING', 'WINDOW', 'ORDER', 'LIMIT',
  'FILTER', 'OVER', 'JOIN', 'ON', 'USING',
  'UNION', 'INTERSECT', 'EXCEPT',
  'THEN', 'ELSE', 'WHEN', 'DO', 'SET', 'VALUES', 'RETURNING',
]);

const TABLE_NAME_CONTEXT_TOKEN_NAMES = new Set([
...
export function enhanceParseError(opts: EnhanceParseErrorOptions): EnhancedParseDiagnostic {
  ...
  const range: [number, number] =
    token.synthetic && token.text.length === 0
...
function buildUnclosedGroupHint(
  token: TokenNode,
...
function previousConcreteToken(
  tokens: readonly TokenNode[],
  tokenIndex: number,
): TokenNode | null {
  for (let i = tokenIndex - 1; i >= 0; i--) {
...
function scanOpenGroups(
  ...
  for (let i = 0; i < tokenIndex; i++) {
    ...
    switch (t.name) {
      case 'LP':   openGroups.push({ kind: 'LP', token: t });   break;
      case 'RP':   popLastGroup(openGroups, 'LP');              break;
      case 'CASE': openGroups.push({ kind: 'CASE', token: t }); break;
      case 'END':  popLastGroup(openGroups, 'CASE');            break;
      default: break;
    }
  }
  return openGroups;
