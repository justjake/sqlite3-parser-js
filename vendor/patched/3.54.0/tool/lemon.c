...
void ReportTableJSON(struct lemon *, const char *);
...
/* Path of the JSON dump file requested with -J, or NULL if not requested. */
static char *outputJsonName = NULL;
static void handle_J_option(char *z){
  outputJsonName = (char *) lemon_malloc( lemonStrlen(z)+1 );
  if( outputJsonName==0 ){
    memory_error();
  }
  lemon_strcpy(outputJsonName, z);
}
...
int main(int argc, char **argv){
  ...
  static struct s_options options[] = {
    ...
    {OPT_FSTR, "J", (char*)handle_J_option,
                    "Emit machine-readable JSON dump of the parser tables."},
  ...
  */
  ...
  if( rpflag ){
    ...
    ** generated parser tables smaller. */
    ...
    /* Optionally emit a machine-readable JSON dump of all the data
    ** that ReportTable just used to generate the C parser. */
    if( outputJsonName ) ReportTableJSON(&lem, outputJsonName);
...
/*
** Emit a JSON-encoded string literal to "out" for the C string "z".
** A NULL pointer is emitted as the JSON literal `null`.
*/
static void json_string(FILE *out, const char *z){
  unsigned char c;
  if( z==0 ){
    fprintf(out, "null");
    return;
...
/* Map symbol_type enum to a stable string label. */
static const char *json_symbol_type(enum symbol_type t){
  switch( t ){
    case TERMINAL:      return "TERMINAL";
    case NONTERMINAL:   return "NONTERMINAL";
    case MULTITERMINAL: return "MULTITERMINAL";
  }
  return "UNKNOWN";
}

/* Map e_assoc enum to a stable string label. */
static const char *json_assoc(enum e_assoc a){
  switch( a ){
...
/* Emit a single rule's RHS as a JSON object describing positions and
** the (possibly multi-)symbols at each position.
*/
static void json_rule_rhs(FILE *out, struct rule *rp){
  int j, k;
  fputc('[', out);
  for(j=0; j<rp->nrhs; j++){
    struct symbol *sp = rp->rhs[j];
    ...
    fprintf(out, "\"pos\":%d,", j);
    fputs("\"alias\":", out);
    json_string(out, rp->rhsalias[j]);
    if( sp->type!=MULTITERMINAL ){
      fprintf(out, ",\"symbol\":%d,\"name\":", sp->index);
      json_string(out, sp->name);
    }else{
      fputs(",\"multi\":[", out);
      for(k=0; k<sp->nsubsym; k++){
        if( k ) fputc(',', out);
        fprintf(out, "{\"symbol\":%d,\"name\":", sp->subsym[k]->index);
        json_string(out, sp->subsym[k]->name);
        fputc('}', out);
      }
      fputc(']', out);
    }
...
/*
** Emit a machine-readable JSON dump of the parser to the file at "path".
** This must be called after ReportTable() (or at least after FindActions
** + CompressTables + ResortStates) so that the action-table compression
** state is finalized.  We rebuild a fresh acttab here rather than
** trying to thread the one used by ReportTable through, to keep this
** function additive and self-contained.
*/
void ReportTableJSON(struct lemon *lemp, const char *path){
  FILE *out;
  struct state *stp;
  struct action *ap;
  struct rule *rp;
  ...
  /* ------------------------------------------------------------------
  ** Re-run the action table compression so that we have local
  ** copies of yy_action[] and yy_lookahead[] to dump.  ReportTable()
  ** has already populated stp->iTknOfst / iNtOfst / iDfltReduce on
  ** every state, but its acttab was freed.  The compression below
  ** is deterministic, so re-executing it produces the same offsets.
  ** ------------------------------------------------------------------ */
  ax = (struct axset *) lemon_calloc(lemp->nxstate*2, sizeof(ax[0]));
  if( ax==0 ){
    fprintf(stderr,"malloc failed\n");
  ...
  /* Compute the YY_SHIFT_COUNT / YY_REDUCE_COUNT row counts the same
  ** way ReportTable does, by trimming trailing NO_OFFSET entries. */
  nShift = lemp->nxstate;
  while( nShift>0 && lemp->sorted[nShift-1]->iTknOfst==NO_OFFSET ) nShift--;
  nReduce = lemp->nxstate;
  while( nReduce>0 && lemp->sorted[nReduce-1]->iNtOfst==NO_OFFSET ) nReduce--;

  /* Min/max destructor token range, mirroring ReportTable. */
  mnDestructor = mxDestructor = 0;
  for(i=0; i<lemp->nsymbol; i++){
    struct symbol *sp = lemp->symbols[i];
    if( sp && sp->type!=TERMINAL && sp->destructor ){
      if( mnDestructor==0 || sp->index<mnDestructor ) mnDestructor = sp->index;
      if( sp->index>mxDestructor ) mxDestructor = sp->index;
    }
  }
  if( lemp->tokendest ) mnDestructor = 0;
  if( lemp->vardest ) mxDestructor = lemp->nsymbol-1;
  ...
  ** ------------------------------------------------------------------ */
  fputs("{\n", out);
  fputs("  \"meta\": {\n", out);
  fputs("    \"lemonVersion\": \"1.0\",\n", out);
  fputs("    \"schemaVersion\": 1,\n", out);
  fputs("    \"sourceFile\": ", out);
  json_string(out, lemp->filename);
  fputs(",\n", out);
  fputs("    \"defines\": [", out);
  {
    int first = 1;
  ...
  /* ------------------------------------------------------------------
  ** Top-level grammar parameters
  ** ------------------------------------------------------------------ */
  fputs("  \"name\": ", out);          json_string(out, lemp->name);          fputs(",\n", out);
  fputs("  \"tokenPrefix\": ", out);   json_string(out, lemp->tokenprefix);   fputs(",\n", out);
  fputs("  \"tokenType\": ", out);     json_string(out, lemp->tokentype);     fputs(",\n", out);
  fputs("  \"varType\": ", out);       json_string(out, lemp->vartype);       fputs(",\n", out);
  fputs("  \"start\": ", out);         json_string(out, lemp->start);         fputs(",\n", out);
  fputs("  \"stackSize\": ", out);     json_string(out, lemp->stacksize);     fputs(",\n", out);
  fputs("  \"reallocFunc\": ", out);   json_string(out, lemp->reallocFunc);   fputs(",\n", out);
  fputs("  \"freeFunc\": ", out);      json_string(out, lemp->freeFunc);      fputs(",\n", out);
  fputs("  \"stackSizeLimit\": ", out);json_string(out, lemp->stackSizeLimit);fputs(",\n", out);
  fputs("  \"extraArg\": ", out);      json_string(out, lemp->arg);           fputs(",\n", out);
  fputs("  \"extraContext\": ", out);  json_string(out, lemp->ctx);           fputs(",\n", out);

  /* ------------------------------------------------------------------
  ** Constants (mirrors the YY*-prefixed #defines emitted by ReportTable)
  ** ------------------------------------------------------------------ */
  fputs("  \"constants\": {\n", out);
  fprintf(out, "    \"YYNSTATE\": %d,\n",            lemp->nxstate);
  fprintf(out, "    \"YYNRULE\": %d,\n",             lemp->nrule);
  fprintf(out, "    \"YYNRULE_WITH_ACTION\": %d,\n", lemp->nruleWithAction);
  fprintf(out, "    \"YYNTOKEN\": %d,\n",            lemp->nterminal);
  fprintf(out, "    \"YYNSYMBOL\": %d,\n",           lemp->nsymbol);
  fprintf(out, "    \"YY_MAX_SHIFT\": %d,\n",        lemp->nxstate-1);
  fprintf(out, "    \"YY_MIN_SHIFTREDUCE\": %d,\n",  lemp->minShiftReduce);
  fprintf(out, "    \"YY_MAX_SHIFTREDUCE\": %d,\n",  lemp->minShiftReduce + lemp->nrule - 1);
  fprintf(out, "    \"YY_ERROR_ACTION\": %d,\n",     lemp->errAction);
  fprintf(out, "    \"YY_ACCEPT_ACTION\": %d,\n",    lemp->accAction);
  fprintf(out, "    \"YY_NO_ACTION\": %d,\n",        lemp->noAction);
  fprintf(out, "    \"YY_MIN_REDUCE\": %d,\n",       lemp->minReduce);
  fprintf(out, "    \"YY_MAX_REDUCE\": %d,\n",       lemp->minReduce + lemp->nrule - 1);
  ...
  fprintf(out, "    \"YY_MIN_DSTRCTR\": %d,\n",      mnDestructor);
  fprintf(out, "    \"YY_MAX_DSTRCTR\": %d,\n",      mxDestructor);
  fprintf(out, "    \"YYWILDCARD\": %d,\n",
                lemp->wildcard ? lemp->wildcard->index : -1);
  fprintf(out, "    \"YYERRORSYMBOL\": %d,\n",
                (lemp->errsym && lemp->errsym->useCnt) ? lemp->errsym->index : -1);
  fprintf(out, "    \"YYFALLBACK\": %d\n",           lemp->has_fallback ? 1 : 0);
  fputs("  },\n", out);

  /* ------------------------------------------------------------------
  ** Preamble / handler code blocks (raw C, included for reference)
  ** ------------------------------------------------------------------ */
  fputs("  \"preamble\": ", out);      json_string(out, lemp->include);  fputs(",\n", out);
  fputs("  \"syntaxError\": ", out);   json_string(out, lemp->error);    fputs(",\n", out);
  fputs("  \"stackOverflow\": ", out); json_string(out, lemp->overflow); fputs(",\n", out);
  fputs("  \"parseFailure\": ", out);  json_string(out, lemp->failure);  fputs(",\n", out);
  fputs("  \"parseAccept\": ", out);   json_string(out, lemp->accept);   fputs(",\n", out);
  fputs("  \"extraCode\": ", out);     json_string(out, lemp->extracode);fputs(",\n", out);
  fputs("  \"tokenDestructor\": ", out); json_string(out, lemp->tokendest); fputs(",\n", out);
  fputs("  \"defaultDestructor\": ", out); json_string(out, lemp->vardest); fputs(",\n", out);

  /* ------------------------------------------------------------------
  ** Symbols (full table, terminals first, then nonterminals)
  ** ------------------------------------------------------------------ */
  fputs("  \"symbols\": [", out);
  for(i=0; i<lemp->nsymbol; i++){
    struct symbol *sp = lemp->symbols[i];
    if( i ) fputc(',', out);
    fputs("\n    {", out);
    fprintf(out, "\"id\":%d,\"name\":", sp->index);
    json_string(out, sp->name);
    fprintf(out, ",\"type\":\"%s\"", json_symbol_type(sp->type));
    fprintf(out, ",\"isTerminal\":%s", i<lemp->nterminal ? "true" : "false");
    if( sp->fallback ){
      fprintf(out, ",\"fallback\":%d", sp->fallback->index);
    }
    if( sp->datatype ){
      fputs(",\"datatype\":", out);
      json_string(out, sp->datatype);
    }
    fprintf(out, ",\"dtnum\":%d", sp->dtnum);
    if( sp->prec>=0 ){
      fprintf(out, ",\"precedence\":%d,\"assoc\":\"%s\"",
              sp->prec, json_assoc(sp->assoc));
    }
    if( sp->destructor ){
      fputs(",\"destructor\":", out);
      json_string(out, sp->destructor);
      fprintf(out, ",\"destLineno\":%d", sp->destLineno);
    }
    fprintf(out, ",\"useCnt\":%d", sp->useCnt);
    fprintf(out, ",\"lambda\":%s", sp->lambda ? "true" : "false");
    if( sp->type==MULTITERMINAL ){
      fputs(",\"subsym\":[", out);
      for(j=0; j<sp->nsubsym; j++){
        if( j ) fputc(',', out);
        fprintf(out, "%d", sp->subsym[j]->index);
      }
      fputc(']', out);
    }
    fputc('}', out);
  ...
  /* ------------------------------------------------------------------
  ** Rules (with action C bodies preserved for debugging/inspection)
  ** Iterated in iRule order so that consumers can index by rule id.
  ** ------------------------------------------------------------------ */
  fputs("  \"rules\": [", out);
  for(i=0, rp=lemp->rule; rp; rp=rp->next, i++){
    if( i ) fputc(',', out);
    fputs("\n    {", out);
    fprintf(out, "\"id\":%d", rp->iRule);
    fprintf(out, ",\"lhs\":%d,\"lhsName\":", rp->lhs->index);
    json_string(out, rp->lhs->name);
    if( rp->lhsalias ){
      fputs(",\"lhsAlias\":", out);
      json_string(out, rp->lhsalias);
    }
    fprintf(out, ",\"nrhs\":%d", rp->nrhs);
    fputs(",\"rhs\":", out);
    json_rule_rhs(out, rp);
    fprintf(out, ",\"line\":%d,\"ruleLine\":%d", rp->line, rp->ruleline);
    if( rp->precsym ){
      fprintf(out, ",\"precSymbol\":%d", rp->precsym->index);
    }
    fprintf(out, ",\"noCode\":%s",      rp->noCode      ? "true" : "false");
    fprintf(out, ",\"doesReduce\":%s",  rp->doesReduce  ? "true" : "false");
    fprintf(out, ",\"canReduce\":%s",   rp->canReduce   ? "true" : "false");
    fprintf(out, ",\"neverReduce\":%s", rp->neverReduce ? "true" : "false");
    fprintf(out, ",\"lhsStart\":%s",    rp->lhsStart    ? "true" : "false");
    fputs(",\"actionC\":", out);     json_string(out, rp->code);
    fputs(",\"codePrefix\":", out);  json_string(out, rp->codePrefix);
    fputs(",\"codeSuffix\":", out);  json_string(out, rp->codeSuffix);
    fputc('}', out);
  }
  fputs("\n  ],\n", out);

  /* ------------------------------------------------------------------
  ** Compressed parsing tables (mirrors the C arrays of the same name)
  ** ------------------------------------------------------------------ */
  fputs("  \"tables\": {\n", out);
  ...
  /* yy_lookahead (with the trailing safety-pad entries that ReportTable adds) */
  n = acttab_lookahead_size(pActtab);
  nLookAhead = lemp->nterminal + lemp->nactiontab;
  if( nLookAhead < n ) nLookAhead = n;
  fputs("    \"yy_lookahead\": [", out);
  for(i=0; i<nLookAhead; i++){
    int la;
    if( i<n ){
  ...
  /* yy_shift_ofst (one entry per state up to nShift) */
  fputs("    \"yy_shift_ofst\": [", out);
  for(i=0; i<nShift; i++){
    int ofst = lemp->sorted[i]->iTknOfst;
    if( ofst==NO_OFFSET ) ofst = lemp->nactiontab;
    if( i ) fputc(',', out);
    if( (i % 16)==0 ) fputs("\n      ", out);
    fprintf(out, "%d", ofst);
  ...
  /* yy_reduce_ofst (one entry per state up to nReduce) */
  fputs("    \"yy_reduce_ofst\": [", out);
  for(i=0; i<nReduce; i++){
    int ofst = lemp->sorted[i]->iNtOfst;
    if( ofst==NO_OFFSET ) ofst = mnNtOfst - 1;
    if( i ) fputc(',', out);
    if( (i % 16)==0 ) fputs("\n      ", out);
    fprintf(out, "%d", ofst);
  ...
  /* yy_default (one entry per state) */
  fputs("    \"yy_default\": [", out);
  for(i=0; i<lemp->nxstate; i++){
    stp = lemp->sorted[i];
    int dflt = (stp->iDfltReduce<0)
                  ? lemp->errAction
  ...
  /* yyFallback (only emitted when the grammar declared %fallback) */
  if( lemp->has_fallback ){
    fputs(",\n    \"yyFallback\": [", out);
    for(i=0; i<lemp->nterminal; i++){
      struct symbol *sp = lemp->symbols[i];
      if( i ) fputc(',', out);
      if( (i % 16)==0 ) fputs("\n      ", out);
      fprintf(out, "%d", sp->fallback ? sp->fallback->index : 0);
    }
    fputs("\n    ]", out);
  }
  fputs("\n  }\n", out);
