...
/*
** Optional path supplied by `-J<path>`.  When set, in addition to (or
** instead of) emitting the C source on stdout, mkkeywordhash writes a
** JSON description of the keyword table to this file.  This is the
** machine-readable handoff to the JS-side codegen.
*/
static const char *g_jsonOut = 0;
...
/* ----------------------------------------------------------------------
** JSON dump support.  Mirrors the additive `-J<path>` flag added to
** tool/lemon.c.  We emit a list of every keyword as authored, with the
...
** name.  This list must be kept in sync with those #defines.  Any bit
** that is not in this table is emitted as a hex literal ("0x...") so
** that future SQLite versions adding new feature flags still produce a
** valid (if slightly anonymous) dump.
*/
typedef struct MaskName MaskName;
struct MaskName { const char *zName; int bit; };
static const MaskName aMaskNames[] = {
  { "ALTER",      0x00000001 },
  { "ALWAYS",     0x00000002 },
  { "ANALYZE",    0x00000004 },
  { "ATTACH",     0x00000008 },
  ...
  { "VIEW",       0x00008000 },
  { "VTAB",       0x00010000 },
  { "AUTOVACUUM", 0x00020000 },
  { "CTE",        0x00040000 },
  { "UPSERT",     0x00080000 },
  { "WINDOWFUNC", 0x00100000 },
  { "GENCOL",     0x00200000 },
  { "RETURNING",  0x00400000 },
  { "ORDERSET",   0x00800000 },
};
#define N_MASK_NAMES ((int)(sizeof(aMaskNames)/sizeof(aMaskNames[0])))

/* JSON-string-escape `z` and emit it (or the literal `null` for a NULL
** pointer).  Same shape as the helper in lemon.c so output stays
** uniform across the two tools.
*/
static void emit_json_string(FILE *out, const char *z){
  unsigned char c;
  if( z==0 ){
    fputs("null", out);
    return;
...
static void emit_keyword_json(FILE *out, const Keyword *p){
  ...
  rest = p->mask;
  first = 1;
  for(j=0; j<N_MASK_NAMES; j++){
    if( (rest & aMaskNames[j].bit) != 0 ){
      if( !first ) fputs(", ", out);
      emit_json_string(out, aMaskNames[j].zName);
      rest &= ~aMaskNames[j].bit;
      first = 0;
    }
  }
  if( rest != 0 ){
    /* Future-proofing: any unknown bits (added in a newer SQLite) are
    ** preserved as a hex literal so consumers can still see them. */
    if( !first ) fputs(", ", out);
    fprintf(out, "\"0x%x\"", rest);
  }
  fputs("]}", out);
}

/* Walk aKeywordTable[] and write the JSON dump.  Must be called BEFORE
** main() filters out keywords whose mask is 0 (so the dump captures the
** complete keyword set as authored, regardless of how this binary was
** compiled).
*/
static void dump_keywords_json(const char *path,
                               const Keyword *table, int n){
  ...
  fputs("{\n", out);
  fputs("  \"meta\": {\n", out);
  fputs("    \"sourceFile\": \"tool/mkkeywordhash.c\",\n", out);
  fprintf(out, "    \"schemaVersion\": 1,\n");
  fprintf(out, "    \"keywordCount\": %d,\n", n);
  fputs("    \"maskFlags\": {\n", out);
  for(i=0; i<N_MASK_NAMES; i++){
    fprintf(out, "      \"%s\": %d%s\n",
            aMaskNames[i].zName, aMaskNames[i].bit,
            i==N_MASK_NAMES-1 ? "" : ",");
  }
  fputs("    }\n", out);
  fputs("  },\n", out);
  ...
  for(i=0; i<n; i++){
    if( !first ) fputs(",\n", out);
    emit_keyword_json(out, &table[i]);
    first = 0;
  }
  fputs("\n  ]\n", out);
  fputs("}\n", out);
  fclose(out);
  /* j is unused outside the inner emit; silence -Wunused on some compilers. */
  (void)j;
}
...
int main(int argc, char **argv){
  ...
  /* Parse args.  Lemon-style: `-J<path>` selects JSON dump mode.  Any
  ** unrecognised arg is reported but doesn't abort, since legacy callers
  ** invoke this binary with no args at all. */
  for(i=1; i<argc; i++){
    const char *a = argv[i];
    if( a[0]=='-' && a[1]=='J' ){
      g_jsonOut = &a[2];
    }else{
      fprintf(stderr, "warning: ignoring unrecognised arg \"%s\"\n", a);
    }
  }

  /* Emit the JSON dump (if requested) BEFORE the mask==0 filter that
  ** follows, so the dump always contains the complete keyword set as
  ** authored — independent of which SQLITE_OMIT_* flags this binary
  ** was compiled with. */
  if( g_jsonOut ){
    dump_keywords_json(g_jsonOut, aKeywordTable, nKeyword);
  }
